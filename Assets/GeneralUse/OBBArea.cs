...
public struct OBBArea
{
    ...
    public static bool CheckCollision(OBBArea area1, OBBArea area2)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (useY) {
                a1 = PointProjectionOnLine(area1Corners[0], axes[i]).y;
                a2 = PointProjectionOnLine(area2Corners[0], axes[i]).y;
            }
            else {
                a1 = PointProjectionOnLine(area1Corners[0], axes[i]).x;
                a2 = PointProjectionOnLine(area2Corners[0], axes[i]).x;
            }
            ...
            for (int j = 1; j < 4; j++)
            {
                ...
                if (useY) {
                    a1 = PointProjectionOnLine(area1Corners[j], axes[i]).y;
                    a2 = PointProjectionOnLine(area2Corners[j], axes[i]).y;
                }
                else {
                    a1 = PointProjectionOnLine(area1Corners[j], axes[i]).x;
                    a2 = PointProjectionOnLine(area2Corners[j], axes[i]).x;
                }
    ...
    static Vector2 PointProjectionOnLine(Vector3 point, Vector3 line)
    {
    ...
    static Vector2 PointProjectionOnLine(Vector2 point, Vector2 line)
    {
        return PointProjectionOnLine(new Vector3(point.x, point.y), new Vector3(line.x, line.y));
    }
